...
count = 0;
const serveTaskName = function(req, res, next) {
  const taskName = JSON.parse(fs.readFileSync('./dataBase/taskName.json', 'utf8'));
  taskName.push(JSON.parse(req.body));
  const content = JSON.stringify(taskName, null, 2);
  fs.writeFileSync('./dataBase/taskName.json', content);
